...
### 🚀 實戰進階：如何一鍵自動掃描「全網站」的個資與敏感資訊外洩？

> 💡 **核心運作原理（爬蟲 + 被動規則連動）**：
> ZAP 的被動掃描腳本是**全域常駐**在流量管道上的。這意味著：**只要任何請求經過 ZAP，腳本就會自動檢查**！
> 因此，我們不需要手動點擊每一個網頁，只要結合 **Spider (全站爬蟲)**，ZAP 就會自動走訪全網站所有連結與頁面，並在幾十秒內自動完成全站個資外洩大盤點！

#### 🎯 全自動掃描全站個資外洩 4 步驟：

1. **步驟 1：確認個資腳本已啟用**
   - 確認左側目錄樹 `Passive Rules` 底下的 `PII_Detector` 圖示帶有 **綠色播放箭頭**（已啟用）。
2. **步驟 2：對目標根網域啟動全站爬蟲 (Spider)**
   - 在左側 **Sites (站台)** 面板中，找到目標根節點 **`http://localhost:8080`**。
   - 在其上方按滑鼠右鍵 -> 選擇 **Attack (攻擊) > Spider... (網頁爬蟲)**。
3. **步驟 3：開始爬行**
   - 在彈出的視窗中點選 **Start Scan (開始掃描)**。
   - ZAP 爬蟲將以極高速度自動遍歷網站的所有子頁面、超連結與靜態資源。
4. **步驟 4：檢視全站外洩報告**
   - 爬蟲執行期間與完成後，點選下方 **Alerts (警示)** 面板。
   - 展開 **`自訂個資洩漏：偵測到身分證字號`** 或 **`自訂個資洩漏：偵測到手機號碼`**。
   - 您會看到**全網站所有存在個資外洩問題的 URL 清單、觸發的明文身分證字號與證據**，全部被一次性完整列出！

---

#### 🔐 進階情境：掃描「需要登入後才能看見」的內部頁面個資
若個資頁面位於登入後的後台或會員中心：
1. 匯入專案的 **`zap/VulnCampus_Default.context`**（詳見第 5 節）。
2. 在頂端工具列點選 **鎖頭圖示 (Forced User Mode)**，選定測試帳號 `student01`。
3. 再次對 `http://localhost:8080` 執行 **Spider (爬蟲)**。
4. ZAP 將自動保持登入狀態爬取所有受保護的會員頁面，並自動抓出所有內部頁面的個資外洩！
